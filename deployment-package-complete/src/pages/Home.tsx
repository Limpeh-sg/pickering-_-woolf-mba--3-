          ...
          >
            ...
            <motion.div variants={revealItem} className="relative">
              ...
              <motion.img
                ...
                className="relative z-10 w-full rounded-[4rem] shadow-2xl transition-transform duration-700" 
                alt="Overview" 
                loading="lazy"
                decoding="async"
              />
  ...
  if (compact) {
    ...
    return (
      <form onSubmit={handleSubmit} className="flex flex-wrap items-center gap-2">
        <input name="firstName" required type="text" placeholder={lang === 'en' ? 'First Name' : '名'} className={`${compactInput} w-24`} />
        <input name="email" required type="email" placeholder={lang === 'en' ? 'Email' : '邮箱'} className={`${compactInput} w-40`} />
        <input name="phone" required type="tel" placeholder={lang === 'en' ? 'Phone' : '电话'} className={`${compactInput} w-32`} />
        <select name="experience" required className={`${compactSelect} w-36`} defaultValue="">
        ...
        <button type="submit" className="shrink-0 bg-primary text-white text-[10px] font-black uppercase tracking-widest px-4 py-2 hover:bg-primary/90 transition-colors">
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 text-left">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <input name="firstName" required type="text" placeholder={lang === 'en' ? 'First Name' : '名字'} className={inputClass} />
        <input name="lastName" required type="text" placeholder={lang === 'en' ? 'Last Name' : '姓氏'} className={inputClass} />
      </div>
      <input name="email" required type="email" placeholder={lang === 'en' ? 'Email Address' : '邮箱地址'} className={inputClass} />
      <input name="phone" required type="tel" placeholder={lang === 'en' ? 'Phone / WhatsApp' : '电话 / WhatsApp'} className={inputClass} />
      <select name="experience" required className={selectClass} defaultValue="">
      ...
      <select name="industry" required className={selectClass} defaultValue="">
        <option value="" disabled>{lang === 'en' ? 'Select Industry...' : '选择行业...'}</option>
        {compactIndustryOptions.map((option) => (
          <option key={option.value} value={option.value}>{lang === 'en' ? option.en : option.zh}</option>
        ))}
      </select>
      <button className="cta-button w-full bg-primary text-white shadow-lg shadow-primary/20 hover:bg-secondary">
        {lang === 'en' ? 'Get in Touch' : '联系顾问'}
      </button>
  ...
  return (
    ...
    >
      <img
        ...
        alt={cleanTitle}
        className="-mx-10 -mt-10 md:-mx-12 md:-mt-12 mb-8 h-44 w-[calc(100%+5rem)] md:w-[calc(100%+6rem)] object-cover grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-700"
        loading="lazy"
        decoding="async"
      />
